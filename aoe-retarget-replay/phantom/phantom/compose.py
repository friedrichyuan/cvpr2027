...
binary mask, which leaves three visible artifacts:

  1. *Color mismatch* — robot is neutral gray, the scene is warm; the robot
     looks pasted on.
  2. *Hard 1-pixel edges* — the segmentation buffer is binary, so silhouettes
     jaggy, and the RGB ring carries a black tint from the render bg.
  3. *No contact shadow* — robot hands float without grounding.

M3 fixes (1) with Reinhard mean+std harmonization in LAB sampled from a ring
around the robot, (2) with feathered alpha + a separate ring de-fringing step
(applied to the robot RGB before composition), and (3) with a depth-gated
feathered drop shadow of the Dex3 hand mask.

All helpers are pure numpy + cv2 — no torch — so this module is cheap to import
and runs on the same host that does the SAM2/E2FGVI work.
"""
...
def harmonize_color(
    ...
    scene_ema: Optional[SceneStatsEMA] = None,
) -> np.ndarray:
...
def extend_robot_into_bleed_zone(
    ...
    composited edge looks cold/white — at odds with the warm robot interior.
...
def compose(
    ...
    *,
    ...
    """One-shot compositor: cleans robot edge, harmonizes color, draws
