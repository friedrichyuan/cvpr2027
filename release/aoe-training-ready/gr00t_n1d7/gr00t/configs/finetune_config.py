...
class FinetuneConfig:
    ...
    vlm_model_name: str = "nvidia/Cosmos-Reason2-2B"
    """
    HuggingFace model ID or local path for the VLM backbone (tokenizer + visual processor).
    Default is the Cosmos-Reason2-2B model used by GR00T N1.7. Pass a local path to
    avoid downloading from HuggingFace Hub on every run, e.g. /data/Cosmos-Reason2-2B.
    """
    ...
    """DeepSpeed ZeRO stage. Stage 2 (optimizer sharding) or Stage 3 (full parameter + optimizer + gradient sharding).
    Use stage 3 for consumer GPUs with limited VRAM (e.g. RTX 4090 24GB)."""

    gradient_checkpointing: bool = True
    """If True, enable gradient/activation checkpointing to reduce VRAM at the cost of extra forward passes.
    Recommended for full-parameter pretraining on consumer GPUs."""
