"""Train an ARX policy that transitions from zero pose to EgoDex frame zero."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="TensorBoard/checkpoint directory (default: timestamped logs/arx_transition)",
    )
...
def main() -> None:
    ...
    log_dir = args.log_dir or (
        Path("logs/arx_transition") / datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    log_dir = log_dir.expanduser().resolve()
    log_dir.mkdir(parents=True, exist_ok=True)
    env_cfg = make_transition_env_cfg(
        args.reference_dir,
        args.num_envs,
    ...
    cfg = RslRlOnPolicyRunnerCfg(
        ...
        logger="tensorboard",
        experiment_name="arx_initial_transition",
    )
