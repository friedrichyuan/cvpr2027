"""Generate a zero-to-EgoDex-prefix plus per-frame ARX IK trajectory with cuRobo."""
...
from .impaint import _make_prefix, _print_summary


def generate_curobo_trajectory(
    episode_path: Path,
    scene_path: Path,
    output_path: Path,
    ...
    used_base = solver.stats.base_offset if solver.stats.base_offset is not None else base_offset
    ...
    metadata = {
        ...
        "base_offset": {
            "dx": float(used_base[0]),
            "dy": float(used_base[1]),
            "yaw_rad": float(used_base[2]),
            "yaw_deg": float(np.degrees(used_base[2])),
        },
        "smoothing_window": None if no_smoothing else smoothing_window,
        "ik_solver": f"curobo_{mode}",
        "curobo_setup_s": solver.stats.setup_s,
        "curobo_solve_s": solver.stats.solve_s,
        "curobo_warmup_s": solver.stats.warmup_s,
        "joint_losses": solver.stats.joint_losses,
    }
    np.savez_compressed(
        output_path,
    ...
    print(
        f"cuRobo {mode}: setup {solver.stats.setup_s:.3f}s | "
        f"warmup {solver.stats.warmup_s:.3f}s | solve {solver.stats.solve_s:.3f}s | "
        f"{ik.qpos.shape[0] / max(solver.stats.solve_s, 1.0e-9):.1f} fps"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("seq", "batch", "trajopt", "mpc", "joint"), default="seq")
